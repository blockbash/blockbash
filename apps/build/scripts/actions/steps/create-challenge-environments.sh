...
main() {
  ...
  log_info "challenge_environment_dir_path: ${challenge_environment_dir_path}"
  log_info "target_arch that we are building for: ${target_arch}"

  local matrix_guid="${target_arch}-${challenge_environment_dir_path}"
  ...
  image_name_short="$(get_challenge_metadata_value "${key_image_name_short}" "${challenge_guid_bash}")"
...
COMMENT
  ...
  local build_flags
  build_flags=(
    --workspace-folder="$(get_challenge_specific_artifact_dir_path "${challenge_guid_bash}")"
    --config="$(get_devcontainer_artifact_file_path "${challenge_guid_bash}" "${build_time_prefix}")"
    --image-name="${image_name_with_branch_arch_full}"
    --cache-from "$(get_cache_flag "${production_branch}" "${docker_challenge_base_image_name_short}" "${target_arch}" "${docker_cache_from}")"
    --cache-from "$(get_cache_flag "${branch_name}" "${docker_challenge_base_image_name_short}" "${target_arch}" "${docker_cache_from}")"
  ...
  # Dont template these values into build-time.devcontainer.base.template.json as we want to allow certain values to be generated from create-challenge-environments.sh.
  ...
  BLOCKBASH_IS_LOCAL="${BLOCKBASH_IS_LOCAL}" \
    ...
    challenge_release=${challenge_release} \
    challenge_repo_name_full=${challenge_repo_name_full} \
  ...
  create_matrix_docker_outputs "${key_create_challenge_images_job}" "${image_name_with_branch_full}" "${branch_name}" "${image_name_short}" "${target_arch}"
  create_github_workflow_outputs "${branch_name}" "${image_name_short}" "${target_arch}" "${challenge_repo_name_full}" "${matrix_guid}"
